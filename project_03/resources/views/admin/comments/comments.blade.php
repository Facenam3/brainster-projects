<x-app-layout>
        ...
        <div class="py-12 px-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex justify-between align-center">
                    <h1 class="p-4 text-xl text-orange-500 font-medium ">Comment</h1>
                    <a href="{{ route('admin.comment-form') }}"
                ...
                <div class="body  border border-gray-400 rounded-md mt-3">
                    <form method="GET" action="#" class="p-4 bg-white rounded-md">
                        <input type="text" name="search" class="rounded-md mx-2" placeholder="Search comment..."
                        ...
                        <tbody>
                            @if ($comments)
                                @foreach ($comments as $comment)
                                    <tr class="border-b-2">
                                        <td class="px-6 py-3">
                                            {{ Str::limit($comment->comment_body, 15) }}
                                        </td>
                                        <td class="px-6 py-3">
                                            {{ $comment->user->name }}
                                        </td>
                                        ...
                                        <td>
                                            <a href="#" data-id="{{ $comment->id }}" class="text-blue-600 mx-2"
                                                id='edit'>
                                                <i class="fa-solid fa-pen-to-square"></i>
                                                Edit
                                            </a>

                                            <a href="#" data-id="{{ $comment->id }}" class="text-red-600"
                                                id="delete">
                    ...
                    <div class="flex justify-between items-center mx-3 my-3">
                        ...
                        <div>
                            {{ $comments->links('pagination.comments') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).on('click', '#edit', function(e) {
            ...
            window.location.href = `/admin/comment-edit/${commentId}`;
        });
